...
return {
    ...
    methods: {
      ...
      nextTheme(){
        ...
        bbn.cp.defaults.code.theme = themes[themeIndex];
      ...
      phpHint(str, cursor, token){
        ...
        // bbn.cp.phpLang must have been defined by an ajax call n mount
        if (!bbn.cp.phpLang) {
          return;
        }
            ...
            for (let i = 0; i < words.length; i++) {
              ...
              if (i === words.length - 1) {
                if (isFn || isMethod) {
                  ...
                  if (tmp.ref) {
                    let row = bbn.fn.getRow(bbn.cp.phpLang, {name: tmp.ref});
                    if (row && row.items) {
                      all.push(...row.items);
                    }
                  }
      ...
      addDefinition(className, varName) {
        if (!bbn.fn.getRow(bbn.cp.phpLang, {"name": className})) {
          if (this.definitionUrl) {
            ...
            bbn.fn.post(this.definitionUrl, {"className": className}, d => {
              if (d.success && d.res) {
                bbn.cp.phpLang.push(bbn.fn.extend(d.res, {"ref": varName}));
                if (!bbn.fn.getRow(bbn.cp.phpLang, {"name": varName})) {
                  let ref = {
                    "name": varName,
                    "type": "class",
                    "items": [],
                  }
                  bbn.cp.phpLang.push(ref);
                }
              }
            })
          }
        } else {
          let obj = JSON.parse(JSON.stringify(bbn.fn.getRow(bbn.cp.phpLang, {"name": className})));
          let newObj = bbn.fn.extend(obj, {"name": varName});
          bbn.cp.phpLang.push(newObj);
        }
      },
      ...
      */
      ...
      updateHtmlHints() {
        let components = bbn.cp.known.sort();
        ...
        if (hash !== bbn.var.componentsHash) {
          ...
          bbn.fn.iterate(components, cpName => {
            ...
            let cls = bbn.fn.camelToCss(cpName) + 'Cp';
            if (cls && cls.bbnCfg?.props) {
              bbn.fn.iterate(cls.bbnCfg.props, (prop, propName) => {
     ...
     */
    mounted(){
      if ((this.mode === 'php') && !bbn.cp.phpLang) {
        bbn.fn.ajax({
