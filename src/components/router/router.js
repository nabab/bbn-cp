    ...
    methods: {
      ...
      realRoute(url, st, force, anchor){
        ...
        if (this.urls[st]) {
          ...
          if (idx === false) {
            throw new Error(bbn._("Impossible to find the configuration for the URL %s", url));
          }
          const currentView = this.views[idx];
          // First routing, triggered only once
          if (currentView.pane) {
            let pane = bbn.fn.getRow(this.currentPanes, {id: currentView.pane});
            if (pane && pane.tabs) {
        ...
        if (this.urlNavigation) {
          if (this.parentContainer) {
            ...
            if (!this.parentContainer.isPane) {
              ...
              this.parentContainer.current = this.baseURL + url;
            }
          }
