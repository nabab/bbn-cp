...
bbnBuilder.prototype.props = function(node, hashName) {
  ...
  if (bbn.fn.numProperties(node.attr)) {
    if (node.attr['bbn-bind']) {
      $_tmp1 = setExpResult($_this, node.attr['bbn-bind'].hash, node.attr['bbn-bind'].exp, hashName) || bbn.fn.createObject();
      if (!$_go && (getExpState($_this, node.attr['bbn-bind'].hash, hashName) !== "OK")) {
        $_go = true;
      }
      ...
      for (let n in node.attr) {
        ...
        if (node.attr[n].exp) {
          $_tmp2[n] = setExpResult($_this, node.attr[n].hash, node.attr[n].exp, hashName);
        }
        else {
      ...
      bbn.fn.each(bbn.fn.unique(Object.keys($_tmp1).concat(Object.keys($_tmp2))), n => {
        ...
        if (!$_go && $_node.attr[n] && !Object.hasOwn($_node.attr[n], 'value') && $_node.attr[n].hash && (getExpState($_this, $_node.attr[n].hash, hashName) !== "OK")) {
          $_go = true;
        }
      });
    ...
    // Simpler version
      ...
      for (let n in node.attr) {
        ...
        if (node.attr[n].exp) {
          $_tmp1 = setExpResult($_this, node.attr[n].hash, node.attr[n].exp, hashName);
          if ($_tmp1 !== undefined) {
            if (n === 'class') {
