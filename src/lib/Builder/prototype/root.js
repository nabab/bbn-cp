  ...
  if ((tpl.length === 1)
    ...
    if (tpl[0].attr) {
      ...
      for (let n in tpl[0].attr) {
        //$_props[n] = 
        if (tpl[0].attr[n].exp) {
          setExpResult($_this, tpl[0].attr[n].hash, tpl[0].attr[n].exp, hashName);
        }
        else {
      ...
      if (bbn.fn.numProperties(tpl[0].directives)) {
        ...
        for (let n in tpl[0].directives) {
          ...
          if (tpl[0].directives[n].exp) {
            $_this.$el.bbnSchema.directives[n].value = setExpResult($_this, tpl[0].directives[n].hash, tpl[0].directives[n].exp, hashName);
          }
      ...
      applySchemaOnComponent($_this, $_props);
      if (tpl[0].events) {
        if (!$_this.$numBuild) {
