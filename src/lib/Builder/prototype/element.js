...
import applyPropsOnElement from "../../Cp/private/applyPropsOnElement.js";

bbnBuilder.prototype.loop = async function(node, hashName) {
  if (node.tag) {
    if (node.model) {
      bbn.fn.iterate(node.model, m => {
        setExpResult($_this, m.hash, m.exp, hashName);
        if (!$_go && (getExpState($_this, m.hash, hashName) !== "OK")) {
          $_go = true;
        }
      });
    }
    if (bbn.fn.numProperties(node.directives)) {
      for (let n in node.directives) {
        if (node.directives[n].exp) {
          setExpResult($_this, node.directives[n].hash, node.directives[n].exp, hashName);
          if (!$_go && (getExpState($_this, node.directives[n].hash, hashName) !== "OK")) {
            $_go = true;
          }
        }
      }
      ...
      if ($_anew) {
        if (node.model) {
          for (let n in node.model) {
            $_tmp1.model[n].value = $_tmp1.props[n] = setExpResult($_this, $_node.model[n].hash, node.model[n].exp, hashName);
          }
        }
        if (bbn.fn.numProperties(node.directives)) {
          for (let n in node.directives) {
            if (node.directives[n].exp) {
              $_tmp1.directives[n].value = getExpValue($_this, node.directives[n].hash, hashName);
            }
          }
        }
        $_items[node.id] = await buildElement($_this, $_tmp1, $_par.at(-1), $_num[$_par.at(-1).bbnId], $_tmp1.loop);
          ...
          for (let n in node.model) {
            if (n === '_default_') {
              if ($_this.$isComponent($_items[node.id])) {
                ...
                $_tmp1.model[modelProp].value = $_tmp1.props[modelProp] = setExpResult($_this, $_node.model[n].hash, node.model[n].exp, hashName);
              }
              else {
                $_tmp1.model.value.value = $_tmp1.props.value = setExpResult($_this, $_node.model[n].hash, node.model[n].exp, hashName);
              }
            }
            else {
              $_tmp1.model[n].value = $_tmp1.props[n] = setExpResult($_this, $_node.model[n].hash, node.model[n].exp, hashName);
        ...
        if (bbn.fn.numProperties(node.directives)) {
          for (let n in node.directives) {
            if (node.directives[n].exp) {
              if (getExpState($_this, node.directives[n].hash, hashName) !== "OK") {
                $_node.directives[n].value = getExpValue($_this, node.directives[n].hash, hashName);
                $_items[node.id].bbnSchema.directives[n].value = getExpValue($_this, node.directives[n].hash, hashName);
                bbn.cp.updateDirectives({ n: $_node.directives[n] }, $_items[node.id]);
              }
            }
          }
        }
        applyPropsOnElement($_this, $_tmp1, $_items[node.id]);
      }
      if ($_par.at(-1) === $_this.$el) {
        $_num['-']++;
      }
          ...
          if (node.model) {
            for (let name in node.model) {
              ...
              _bbnCurrentElement.addEventListener(_bbnEventName, _bbnEventObject => {
                ...
                let oldValue = bbn.fn.isPrimitive(modelVarName) ? setExpResult($_this, m.hash, modelVarName, hashName) : modelVarName;
                ...
                if (oldValue !== _bbnEventValue) {
                  if (modelVarRoot === modelVarName) {
                    if (Object.hasOwn($_this.$props, modelVarRoot)) {
                      ...
                      setProp($_this, modelVarRoot, _bbnEventValue);
                    }
                    else {
