...
const treatCondition = (cp, node, arr, hashName) => {
  ...
  bbn.fn.each(tmp, (cond, j) => {
    ...
    if (['template', 'transition', 'slot'].includes(cond.tag)) {
      ...
      x(c, sp, `        comment: true`);
      x(c, sp, `      });`);
      x(c, sp, `      _t.$insertElement(_eles['${cond.id}'], _parents.at(-1));`);
      x(c, sp, `    }`);
    }
...
export default function templateToFunction(cp, tpl, sp = 0) {
  ...
  let c = x();
  sp += 2;
  x(c, sp, `bbn.fn.log(['templateToFunction', cp]); const _r = _t.$currentResult;`);
  x(c, sp, `let ${hashName} = '';`);
  ...
  return new AsyncFunction('_t', '_d', c.text);
