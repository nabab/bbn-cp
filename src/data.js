class bbnData {
  static getCpKey(component) {
    return component.$id + '---' + (component.$hash ? '---' + component.$hash : '');
  }
  ...
  static treatValue(value, component, path) {
    if (value && (typeof value === 'object') && [undefined, Object, Array].includes(value.constructor)) {
      if (value.__bbnData) {
      ...
      return new bbnData(value, component, path);
    }
  ...
  static proxyPush(target, component, path) {
    return (...args) => {
      ...
      bbn.fn.each(args, (a, i) => {
        ...
        newPath.push(idx);
        const newVal = bbnData.treatValue(a, component, newPath);
        newArgs.push(newVal === a ? a : newVal.value);
      });
  ...
  static proxySplice(target, component, path) {
    return (index, numDelete, ...args) => {
      ...
      for (let i = index; i < index + numDelete; i++) {
        if (target[i] && target[i].__bbnData) {
          dataObj.unset(i);
        }
      }
  ...
  static getProxyHandler(component, path) {
    return {
      get(target, key) {
        const realValue = target[key];
        if (bbn.fn.isFunction(realValue)) {
          const targetObj = component.$values[target.__bbnData];
          if (targetObj && targetObj.isArray && bbn.fn.isString(key)) {
            ...
            if (bbn.fn.isFunction(bbnData[fnName])) {
              return bbnData[fnName](target, component, newPath);
            }
          ...
          const newPath = path.slice();
          newPath.push(key);
          const dataObj = bbnData.treatValue(realValue, component, newPath);
          if (dataObj !== realValue) {
      ...
      set(target, key, value) {
        if (!bbn.fn.isSame(target[key], value)) {
          const newPath = path.slice();
          newPath.push(key);
          const dataObj = bbnData.treatValue(value, component, newPath);
          target[key] = dataObj === value ? value : dataObj.value;
          if
          component.$tick();
        }
      ...
      defineProperty(target, key, description) {
        if (target[key] && target[key].__bbnData) {
          const dataObj = bbn.cp.dataInventory.get(target[key].__bbnData);
          if (!dataObj) {
            throw new Error(bbn._("The data inventory does not contain the data object"));
          }
          dataObj.remove(key);
        ...
        if (description.value) {
          description.value = bbnData.treatValue(description.value, component, newPath);
        }
  ...
  constructor(data, component, path) {
    ...
    if (!bbn.fn.isArray(path) || !path.length) {
      throw new Error("bbnData must be initialized with a path");
    }
    ...
    Object.defineProperty(this, 'value', {
      value: new Proxy(this.data, bbnData.getProxyHandler(component, path)),
      writable: false,
      configurable: false
    });
    ...
    component.$values[this.id] = this;
  }

  unset(prop) {
    if (this.data[prop]?.__bbnData) {
      const dataObj = bbn.cp.dataInventory.get(this.data[prop].__bbnData);
      bbn.fn.each(this.data[prop], (a, n) => {
        if (a?.__bbnData) {
          dataObj.unset(n);
        }
      });
      const id = this.data[prop].__bbnData;
      this.root.$values.delete(id);
      bbn.cp.dataInventory.delete(id);
      this.root.$tick();
      bbn.fn.iterate(dataObj.components, (paths, cid) => {
      ...
      delete this.data[prop].__bbnData;
    }
    else {
      throw new Error(bbn._("The property is not a bbnData"));
    }
  }
  ...
  hasComponent(component, path) {
    if (!(component instanceof bbnCp)) {
      throw new Error("bbnData hasComponent must be called with a bbn component");
    }

    path = JSON.stringify(path);
    return this.components[component.$cid] && this.components[component.$cid].includes(path);
  }
  ...
  removeComponent(component, path) {
    ...
    if (this.components[component.$cid]) {
      if (path) {
        path = JSON.stringify(path);
        let idx = this.components[component.$cid].indexOf(path);
        if (idx > -1) {
  ...
  update() {
    this.root.$tick();
    bbn.fn.iterate(this.components, (paths, cid) => {
      const cp = bbn.cp.getComponent(cid)?.bbn;
      if (!cp) {
        throw new Error(bbn._("Impossible to find the component %s", cid));
      }
...
class bbnCp {
  ...
  $setUpData(name, data, type = 'data') {
    if (!bbn.fn.isString(name) || name) {
      throw new Error(bbn._("The name of the data must be a string"));
    }

    if (!['data', 'props', 'prop'].includes(type)) {
      throw new Error(bbn._("The type of the data must be 'data' or 'prop'"));
    }

    if (this.$values[name]) {
      throw new Error(bbn._("The data %s is already defined", name));
    }

    const dataObj = bbnData.treatValue(data, this, [name]);
    this.values[name] = dataObj === data ? dataObj.value : dataObj;
  }
  ...
  async $launchQueue(force) {
    ...
    if (this.$queue.length && (force || (time - this.$lastLaunch > bbn.cp.tickDelay))) {
      ...
      for (let i = 0; i < queue.length; i++) {
        await a.cp.updateComponent();
        a.fns.forEach(fn => fn.bind(a.cp)());
      }
    }
  ...
  */
  $setData(name, value) {
    const cfg = this.$cfg;
    const dataObj = bbnData.treatValue(value, this, [name]);
    this.$dataValues[name] = dataObj === value ? value : dataObj.value;
    if (!Object.hasOwn(this, name)) {
      ...
      const def = {
        ...
        set(v) {
          let oldV = this.$dataValues[name] && (this.$dataValues[name] instanceof bbnData) ? this.$dataValues[name].value : this.$dataValues[name];
          let hash = v;
          ...
          if (this.$oldValues[name] !== hash) {
            const dataObj = bbnData.treatValue(v, this, [name]);
            this.$dataValues[name] = dataObj === v ? v : dataObj.value;
  ...
  $realSetProp(name, value) {
    ...
    const dataObj = bbnData.treatValue(value, this, [name]);
    const newValue = value === dataObj ? value : dataObj.value;
    Object.defineProperty(this.$props, name, {
      value: newValue,
      writable: false,
      configurable: true
  ...
  $tick() {
    return new Promise(resolve => {
      ...
      this.$queue[0].push({cp: this, fns: [resolve]});
    });
  }
  ...
  $set(obj, prop, value, writable = true, configurable = true) {
    if (bbn.cp.isComponent(obj)) {
      ...
      if (!obj.$namespaces[prop]) {
        obj.$setData(prop, value);
      }
      else if (!bbn.fn.isSame(dataObj === value ? value : dataObj.value, obj[prop])) {
  ...
  $nextTick(fn) {
    return new Promise((resolve) => {
      ...
      this.$queue[0].push({cp: this, fns: [fn || resolve]});
    });
  }
